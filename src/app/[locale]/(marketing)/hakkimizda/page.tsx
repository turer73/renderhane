...
export default async function AboutPage({ params }: PageProps) {
  ...
  const sections = [
    { title: t("missionTitle"), content: t("missionBody") },
    { title: t("howTitle"), content: t("howBody") },
    { title: t("techTitle"), content: t("techBody") },
    { title: t("whoTitle"), content: t("whoBody") },
  ];
      ...
      <main className="flex-1">
        <article className="mx-auto max-w-3xl px-4 py-20 sm:px-6">
          ...
          <div className="mt-14 rounded-2xl border border-border bg-muted/40 p-8 text-center">
            <h2 className="text-xl font-semibold text-foreground">{t("ctaTitle")}</h2>
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{t("ctaBody")}</p>
            <div className="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row">
              <Button asChild>
                <Link href={`/${locale}/login`}>{t("ctaButton")}</Link>
              </Button>
              <Button variant="outline" asChild>
                <Link href={`/${locale}/iletisim`}>{t("contactButton")}</Link>
              </Button>
            </div>
          </div>
        </article>
