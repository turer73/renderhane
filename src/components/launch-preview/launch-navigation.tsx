...
import {loginPath,workspacePath,type Locale} from '@/lib/launch-preview/core';
...
export function LaunchNavigation({locale,current='home',production=false}:{locale:Locale;current?:PreviewTool|'home';production?:boolean}){
 ...
 const primaryHref=signedIn?workspacePath(locale,'img-to-3d'):loginPath(locale,'img-to-3d');
 ...
 <div className="rhl-actions"><ThemeToggle/>{!signedIn&&<Link className="rhl-login" href={loginPath(locale,null)}>{tr?'Giriş yap':'Sign in'}</Link>}<Link className="rhl-btn small" href={primaryHref}>{signedIn?(tr?'Uygulamaya git':'Go to app'):(tr?'Üretmeye başla':'Start creating')} <ArrowRight size={14}/></Link><button ref={toggle} className="rhl-menu-toggle" type="button" aria-expanded="false" aria-controls="rhl-mobile-menu" aria-label={tr?'Menü':'Menu'} onClick={()=>dialog.current?.open?closeMenu():openMenu()}><Menu size={18}/><span>{tr?'Menü':'Menu'}</span></button></div></header></div>
 ...
 <div className="rhl-drawer-bottom"><Link className="rhl-btn" href={primaryHref} onClick={closeMenu}>{signedIn?(tr?'Uygulamaya git':'Go to app'):(tr?'3D üretime başla':'Start creating in 3D')} →</Link><p>{signedIn?(tr?'3D model aracı seçili olarak açılır.':'Opens with the 3D model tool selected.'):(tr?'Üretim için giriş gerekir. Model maliyeti krediyle hesaplanır.':'Sign-in is required. Model cost is credit-based.')}</p></div>
 </dialog>
 </div>;
}
