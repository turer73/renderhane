...
const COPY: Array<[string, string]> = [  ["Önce sana ait veya paylaşma iznin olan sayfanın bağlantısını gir.", "Enter a link to a page you own or have permission to share."],
  ...
  ["İçerik yazıldı. Kalıcı kilitten önce etiket geri okunup doğrulanıyor; etiketi uzaklaştırma.", "Content written. The tag is being read back and verified before permanent locking; keep it in place."],
  ["Yazılan içerik doğrulandı. Etiketi uzaklaştırma; kalıcı kilit uygulanıyor.", "The written content was verified. Keep the tag in place while permanent locking is applied."],
  ["Kalıcı kilit için NDEF okuma, yazma ve kilitleme desteği gerekir.", "Permanent locking requires NDEF read, write, and lock support."],
  ["Yazım sonrası doğrulama zaman aşımına uğradı; kalıcı kilit uygulanmadı.", "Post-write verification timed out; permanent locking was not applied."],
  ["Etiketten geri okunan içerik yazılan NDEF ile eşleşmedi; kalıcı kilit uygulanmadı.", "The content read back from the tag did not match the written NDEF; permanent locking was not applied."],
  ["Yazılan içerik etiketten doğrulanamadı; kalıcı kilit uygulanmadı.", "The written content could not be verified from the tag; permanent locking was not applied."],
  ["İşlem durduruldu; etiket yazıldıysa kilit durumu doğrulanamadı.", "The operation was stopped; if the tag was written, its lock status could not be verified."],
  ["Sayfa arka plana geçti; etiket yazıldıysa kilit durumu doğrulanamadı.", "The page moved to the background; if the tag was written, its lock status could not be verified."],
  ["Sayfa değiştirildi; etiket yazıldıysa kilit durumu doğrulanamadı.", "The page changed; if the tag was written, its lock status could not be verified."],
  ["İçerik değiştirildi; etiket yazıldıysa kilit durumu doğrulanamadı.", "The content changed; if the tag was written, its lock status could not be verified."],
  ["30 saniye içinde yazım sonrası doğrulama tamamlanmadı. Etiket yazıldı ancak kilit uygulanmadı.", "Post-write verification did not finish within 30 seconds. The tag was written, but locking was not applied."],
