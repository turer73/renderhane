...
const COPY: Array<[string, string]> = [  ["Önce sana ait veya paylaşma iznin olan sayfanın bağlantısını gir.", "Enter a link to a page you own or have permission to share."],
  ...
  ["Bu cihaz veya NFC sürücüsü kalıcı kilitleme sunmuyor.", "This device or NFC adapter does not provide permanent locking."],
  ["Toplu yazım", "Bulk writing"],
  ["Aynı içeriği 2–100 etikete, her etiketi ayrı ayrı onaylayarak yaz.", "Write the same content to 2–100 tags, confirming each tag separately."],
  ["Etiket adedi", "Number of tags"],
  ["Toplu yazımı başlat", "Start bulk writing"],
  ["Önceki etiketi uzaklaştır, sıradaki etiketi yaklaştır ve düğmeye bas.", "Remove the previous tag, hold the next tag near the phone, and press the button."],
  ["Oturum başındaki içerik sabittir; alan değişiklikleri sonraki oturuma uygulanır.", "The content captured at the start of the session is fixed; field changes apply to the next session."],
  ["Sıradaki etiketi yaz", "Write next tag"],
  ["Toplu yazımı bitir", "End bulk writing"],
  ["WiFi/WSC yazımı için mevcut projedeki NDEF modülü kullanılmalıdır. Web NFC yalnız uyumlu NDEF etiketlerinde çalışır.", "WiFi/WSC writing requires the project's dedicated NDEF module. Web NFC works only with compatible NDEF tags."],
  ["Kalıcı kilit isteğe bağlı", "Permanent locking is optional"],
  ["Yalnız desteklenen etiketlerde ve açık onaydan sonra uygulanır.", "It is applied only to supported tags after explicit confirmation."],
  ["Kontrollü toplu yazım", "Controlled bulk writing"],
  ["Her yeni etiket ayrı işlemle yazılır ve sayaçta izlenir.", "Each new tag is written in a separate operation and tracked by the counter."],
  ...
  ["Etiketi telefonun arkasına yaklaştır. Yazma ve varsa kilitleme tamamlanana kadar uzaklaştırma.", "Hold the tag near the back of the phone until writing and, if selected, locking finish."],
  ["İçerik yazıldı. Etiketi uzaklaştırma; kalıcı kilit uygulanıyor.", "Content written. Keep the tag in place while permanent locking is applied."],
  ...
  ["etiket kalıcı kilitlendi", "tags permanently locked"],
  ["kilitlendi", "locked"],
  ["kilitlenemedi", "could not be locked"],
  ["Toplu yazım hazır:", "Bulk writing ready:"],
  ["Toplu yazım tamamlandı:", "Bulk writing completed:"],
  ["Toplu yazım tamamlandı.", "Bulk writing completed."],
  ["Toplu yazım bitti:", "Bulk writing ended:"],
  ["İlk etiketi yaklaştır ve “Sıradaki etiketi yaz” düğmesine bas.", "Hold the first tag near the phone and press “Write next tag”."],
  ["Etiketi uzaklaştır ve sıradaki etiketle devam et.", "Remove the tag and continue with the next tag."],
  ["Etiketi uzaklaştır, sıradaki etiketi yaklaştır.", "Remove the tag and hold the next tag near the phone."],
  ["son etiket kalıcı kilitlenemedi:", "the last tag could not be permanently locked:"],
  ...
  ["30 saniye içinde kalıcı kilit tamamlanmadı. Etiket yazıldı ancak kilit durumunu kontrol et.", "Permanent locking did not finish within 30 seconds. The tag was written, but check its lock status."],
  ["Toplu yazım içeriği bulunamadı. Oturumu yeniden başlat.", "Bulk-writing content was not found. Restart the session."],
  ["Son hata:", "Last error:"],
  ["30 saniye içinde etiket algılanmadı. İşlemi yeniden başlatabilirsin.", "No tag was detected within 30 seconds. You can start the operation again."],
