...
function MobileDock({locale}:{locale:Locale}) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(entries => { heroVisible = entries[0].isIntersecting; refresh(); }, {threshold: 0});
    observer.observe(hero);
    const mutations = new MutationObserver(refresh);
    mutations.observe(document.body, {attributes: true, attributeFilter: ['open'], subtree: true});
    mq.addEventListener('change', refresh);
    document.addEventListener('focusin', refresh);
    document.addEventListener('focusout', () => queueMicrotask(refresh));
    window.visualViewport?.addEventListener('resize', refresh);
    refresh();
    return () => { observer.disconnect(); mutations.disconnect(); mq.removeEventListener('change', refresh); document.removeEventListener('focusin', refresh); window.visualViewport?.removeEventListener('resize', refresh); };
  }, []);
  return <div className="rhl-mobile-dock" hidden={!visible} aria-label={tr?'Hızlı işlemler':'Quick actions'}><Link className="rhl-btn" href={loginPath(locale,'img-to-3d')}>{tr?'3D model oluştur':'Create a 3D model'} <span aria-hidden="true">→</span></Link><a href="#rhl-free">{tr?'Ücretsiz araçlar':'Free tools'}</a></div>;
}
  ...
  return <div className="rhl">
    ...
    <div className="rhl-shell">
      ...
      <main id="rhl-main">
        ...
        <section className="rhl-section" id="rhl-paths" tabIndex={-1}><div className="rhl-section-head"><div><span className="rhl-label">{tr?'ÜRETİME ODAKLAN':'FOCUS ON CREATING'}</span><h2>{tr?'Ne üretmek istiyorsun?':'What do you want to create?'}</h2></div><p>{tr?'Önce çıktını seç. Araçlar, işini tamamlamak için yanında.':'Choose the output first. Tools help you complete the task.'}</p></div>
          <div className="rhl-routes">{[
            {key:'model',art:'/launch-preview/home/route-3d.webp',title:tr?'3D model':'3D model',desc:tr?'Formu farklı açılardan incele. Tasarım ve üretim hedefin için modeli değerlendir.':'Inspect the form from different angles. Evaluate it for your design and production needs.',cta:tr?'3D üretime geç':'Create in 3D'},
            {key:'scene',art:'/launch-preview/home/route-scene.webp',title:tr?'Ürün sahnesi':'Product scene',desc:tr?'Ürün fotoğrafını farklı ortam ve kompozisyonlarda sun.':'Present your product photo in different settings and compositions.',cta:tr?'Sahne oluşturmaya geç':'Create a scene'},
            {key:'video',art:'/launch-preview/home/route-video.webp',title:tr?'Tanıtım videosu':'Product video',desc:tr?'Görselini hareketle anlat. Ürünün için yeni bir tanıtım içeriği hazırla.':'Tell the story with motion. Prepare new promotional content for your product.',cta:tr?'Video oluşturmaya geç':'Create a video'},
          ].map(({key,art,title,desc,cta})=><article className={`rhl-route rhl-route-visual ${key==='model'?'featured':''}`} key={key}><div className="rhl-route-art" aria-hidden="true"><Image src={art} width={720} height={600} loading="lazy" alt=""/></div><div className="rhl-route-body"><h3>{title}</h3><p>{desc}</p><Link className="rhl-text-link" href={links[key as keyof typeof links]}>{cta} →</Link></div></article>)}</div>
        </section>
        <section id="rhl-cost" className="rhl-estimator" tabIndex={-1}><div><span className="rhl-label">{tr?'MODELE GÖRE KREDİ':'MODEL-BASED CREDITS'}</span><h2>{tr?'Önce maliyetini gör.':'See the cost first.'}</h2><p>{tr?'Her 3D model aynı krediyle çalışmaz. Model ve işlem adedini seç; tahmini kredi ihtiyacını gör.':'3D models do not all use the same credits. Choose a model and generation count to see an estimate.'}</p><p>{tr?'Bu alan kredi satın almaz ve üretim başlatmaz.':'This calculator does not purchase credits or start generation.'}</p></div><div><div className="rhl-fields"><label>{tr?'3D modeli':'3D model'}<select value={choice} onChange={e=>setChoice(e.target.value)}><option value="">{tr?'Model seçin':'Choose a model'}</option>{options.map(o=><option value={o.key} key={o.key}>{o.label} · {o.credits} {tr?'kredi':'credits'}</option>)}</select></label><label>{tr?'İşlem adedi':'Count'}<input type="number" min={1} max={100} step={1} value={count} onChange={e=>setCount(e.target.value)}/></label></div><div className="rhl-total"><span>{tr?'Tahmini toplam':'Estimated total'}</span><output aria-live="polite">{total === null?'—':`${total} ${tr?'kredi':'credits'}`}</output></div>{selected && total===null && <p className="rhl-warning" role="alert">{tr?'1–100 arasında tam sayı girin.':'Enter an integer from 1 to 100.'}</p>}<p>{tr?'Değerler mevcut model kaydından okunur. Yeniden üretimler ek kredi gerektirebilir. Kesin bedel işlem onayında gösterilmelidir.':'Values come from the existing model registry. Regeneration may require more credits. Confirm the exact cost before submitting.'}</p></div></section>
        <section className="rhl-section" id="rhl-free" tabIndex={-1}><div className="rhl-section-head"><div><span className="rhl-label">{tr?'YARDIMCI ARAÇLAR':'SUPPORTING TOOLS'}</span><h2>{tr?'Küçük işler de burada.':'The smaller tasks, too.'}</h2></div><p>{tr?'Ücretsiz yardımcılar ayrı; 3D, sahne ve video üretimi krediyle.':'Free utilities are separate from credit-based 3D, scene and video generation.'}</p></div><div className="rhl-tool-row rhl-tool-row-visual">{[['arka-plan-kaldirma','/launch-preview/home/tool-bg-remove.webp',tr?'Ücretsiz':'Free',tr?'Arka plan kaldır':'Remove background',tr?'Kayıt olmadan · Günde 3 hak':'No signup · 3 per day'],['qr-kod','/launch-preview/home/tool-qr.webp',tr?'Şekilli QR':'Shaped QR',tr?'Standart QR oluştur':'Create a standard QR',tr?'Ücretsiz · PNG ve SVG':'Free · PNG and SVG'],['nfc-yaz','/launch-preview/home/tool-nfc.webp',tr?'Mobil':'Mobile',tr?'NFC etiketi yaz':'Write an NFC tag',tr?'Uyumlu Android + Chrome':'Compatible Android + Chrome']].map(([slug,art,badge,title,sub])=><Link className="rhl-tool rhl-tool-visual" href={`${previewHome(locale)}/araclar/${slug}`} key={slug}><Image className="rhl-tool-art" src={art} width={240} height={224} loading="lazy" alt="" aria-hidden="true"/><div className="rhl-tool-copy"><span className="rhl-tool-badge">{badge}</span><b>{title}</b><small>{sub}</small></div><span className="rhl-tool-arrow" aria-hidden="true">→</span></Link>)}</div><div className="rhl-artistic-strip"><span className="rhl-strip-icon"><svg aria-hidden="true" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.7" viewBox="0 0 24 24"><path d="m14 6 4-4a2.1 2.1 0 0 1 3 3l-9 9-3-3 5-5Z"/><path d="M10 13c-5-2-2 6-8 7 6 2 10 0 10-5"/></svg></span><p>{tr?'Markana özel bir tasarım mı? ':'Looking for a branded design? '}<Link href={`${previewHome(locale)}/araclar/sanatsal-qr`}>{tr?'Sanatsal QR çalışmalarını incele':'Explore artistic QR'} <span className="rhl-paid-tag">{tr?'Ücretli özel tasarım':'Paid design'}</span> <span aria-hidden="true">→</span></Link></p></div><details className="rhl-audit-note" id="rhl-scope"><summary>{tr?'Bu teslimin kapsamı':'Scope of this preview'}</summary><p>{tr?'Ana sayfa korunur. V3 araç ekranları bu önizlemeye bağlanmıştır. Standart QR tarayıcıda üretilir. AI üretim ve sipariş servisleri varsayılan olarak bağlı değildir. Şişe örneği önceden hazırlanmıştır. NFC fiziksel cihazda test edilmedi.':'The approved homepage is preserved. V3 tools are connected to this preview. Standard QR generation runs locally. Live AI and order services are not connected by default. The bottle sample is prebuilt. NFC hardware is not tested.'}</p></details></section>
      </main>
