...
LAYERS = ("relief_map", "mask", "uv_artwork", "white_mask", "varnish_mask")
...
def validate_submission(data: dict) -> tuple[dict, dict]:
    if not isinstance(data, dict) or set(data) - {"recipe", "layers", "sample", "acknowledge_candidate"}:
    ...
    if data.get("sample") == "calibration-v1":
        if "layers" in data:
            raise ValueError("sample and uploaded layers are mutually exclusive")
