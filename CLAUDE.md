...
## Important Notes
- **npm surumu tuzagi:** CI node 22 + `npm ci` (npm 10) kullanir; lokal npm 11 lockfile'i farkli yazar. Risk sinifi: npm 11 aktif olmayan CPU dallarini (opsiyonel `*-wasm32-wasi` binding'leri) budayip altlarindaki nested entry'leri SILEBILIR → npm 10 `npm ci` EUSAGE ile kirilir. Master 1-11 Tem 2026 arasi `@rolldown/binding-wasm32-wasi` altindaki `@emnapi/*` yuzunden kirmiziydi; rolldown 1.2.x o binding'i biraktigi icin bu somut ornek kapandi, sinif kapanmadi. Kalan WASI binding'lerde risk tasiyan yalniz registry'den cozulen bagimliliklardir (`bundleDependencies` tarball icinde gelir, integrity'si olmaz, budanmaz) — hangileri oldugunu lock'tan oku, buraya yazma. Kural: lockfile degisen her commit sonrasi `npx npm@10 ci --dry-run`; opsiyonel entry silindiyse `npx npm@10 install --package-lock-only` ile geri uret. npm 10/11 arasi yalniz `libc` alani farki zararsizdir, ugrasma.
